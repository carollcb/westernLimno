...
How many lakes have both TP and TN?

```{r what exists TP NO3, echo=FALSE, message=FALSE, warning=FALSE}
dt1_western_summary %>%
  dplyr::select(lagoslakeid, tp_ugl_median, tn_ugl_median)%>%
  drop_na(tp_ugl_median,tn_ugl_median) %>%
  summarise_all(funs(sum(!is.na(.)))) %>%
  rownames_to_column() %>%
  pivot_longer(-1) %>%
  dplyr::select(-rowname) %>%
...
annualFreqTP<-dt1_western %>%
 # filter(lagoslakeid %in% tp_complete_cases) %>%
  ...
  names()
annualFreqTP_names<- dt1_western_summary
...
##TN next
#Get names of lakes
TN_complete_cases<-dt1_western_summary %>%
  drop_na(tn_ugl_median) %>%
  dplyr::select(lagoslakeid, tn_ugl_median)%>%
  pivot_wider(names_from=lagoslakeid, values_from=tn_ugl_median) %>%
  names()

zoomTNplot<-dt1_western %>%
  filter(lagoslakeid %in% TN_complete_cases) %>%
  group_by(lagoslakeid) %>%
  summarize(n_years_sampled=length(unique(year)))%>%
  filter(n_years_sampled>3)%>%
  ggplot(aes(x=n_years_sampled))+
...
dt1_western %>%
  ...
  ggplot(aes(x=n_years_sampled))+
  geom_histogram(binwidth=1, color="black", fill="grey40")+
  labs(title="All lakes with TN data",
       subtitle="Minimum 2 years per lake",
       x="Number of years sampled")+
  annotation_custom(grob=ggplotGrob(zoomTNplot), 
                      ymin = 200, ymax=500,
                    xmin=20,xmax=40)
...
##TN next
#Get names of lakes
TN_complete_cases<-dt1_western_summary_2010s %>%
  drop_na(tn_ugl_median) %>%
  dplyr::select(lagoslakeid, tn_ugl_median)%>%
  pivot_wider(names_from=lagoslakeid, values_from=tn_ugl_median) %>%
  names()
...
dt1_western %>%
  ...
  ggplot(aes(x=n_years_sampled))+
  geom_histogram(binwidth=1, color="black", fill="lightblue")+
  labs(title="All lakes with TN data - 2010-present",
       subtitle="Minimum 2 years per lake",
       x="Number of years sampled")+
  annotation_custom(grob=ggplotGrob(zoomTNplot), 
                      ymin = 40, ymax=80,
...
Total phosphorus and TN concentrations across space
...
# TN_spatial_conc<-
  ...
  annotation_scale(location = "bl", width_hint = 0.5) +
    ...
    coord_sf(xlim = c(-130, -100), ylim = c(32, 49), expand=TRUE)+
  geom_point(data=dt1_western_summary %>%
               drop_na(tn_ugl_median), aes(lake_lon_decdeg, lake_lat_decdeg, fill=log10(tn_ugl_median)),
             size=2.5)+
      scale_shape_manual(values=c(21,25))+
...
For the lakes with TP and TN data, when were most of the samples obtained? (median of all years sampled, grouped by decade)
...
ggplot()+
  ...
  geom_point(data=dt1_western_summary %>%
             ...
             shape=21, size=2.5)+
  scale_fill_manual(values=c("#ffbe0b","#fb5607","#ff006e","#8338ec","#3a86ff"))+
  ggtitle("All sites with TN data")
...
The plots below shows the counts of all the lakes sampled for TP and TN, grouped by when most of their samples were taken
...
##TN next
#Get names of lakes
TN_spatial_lakes<-dt1_western_summary %>%
  drop_na(tn_ugl_median) %>%
  dplyr::select(lagoslakeid, tn_ugl_median)%>%
  pivot_wider(names_from=lagoslakeid, values_from=tn_ugl_median) %>%
  names()
...
#Then get a dataframe with only those lakes and count the number of years each site was sampled. 
dt1_western_TN_obs<-dt1_western %>%
  filter(lagoslakeid %in% TN_spatial_lakes) %>%
  group_by(lagoslakeid) %>%
  mutate(n_years_sampled=length(unique(year))) %>%
  # mutate(year=as.numeric(as.character(year))) %>%
  dplyr::select(lagoslakeid,year,tn_ugl_median) %>%
  drop_na(year)
...
dt1_western_TN_obs%>%
  ...
  scale_fill_viridis_d()+
    theme(legend.position="none",
        axis.title.x=element_blank(),
        axis.text.x=element_text(angle = 45, hjust = 1, size=6))+
  ggtitle("Number of unique lakes sampled for TN each year")+
    geom_text(aes(label = n_lakes_sampled),
              position=position_nudge(y=10), size=3)
...
We see a similar number of lakes by size class with TN data, but as you'll see the spatial and connectivity patterns are different.
...
dt1_western_summary%>%
  ...
  mutate(lake_connectivity_class = factor(lake_connectivity_class,
                                          levels=c("Drainage","DrainageLk",
                                                   ...
                                                   "Terminal","TerminalLk"))) %>%
  group_by(lake_connectivity_class) %>%
  ggplot(aes(x=lake_connectivity_class, y=log10(tn_ugl_median), fill=lake_connectivity_class)) +
  geom_boxplot(outlier.shape = NA)+
    theme(legend.position="none",
        axis.title.x=element_blank(),
        axis.text.x=element_text(angle = 45, hjust = 1, size=10))+
...
grouped_gghistostats(
  data = dt1_western_summary %>%
    drop_na(tn_ugl_median) %>%
                filter(lake_connectivity_class %in% c("Drainage","DrainageLk",
                                                   "Isolated")) %>%
                mutate(lake_connectivity_class = factor(lake_connectivity_class,
                                                        levels=c("Isolated","Drainage","DrainageLk"))),
  x = lake_elevation_m,
  # test.value = 50,
  # type = "nonparametric",
  results.subtitle=FALSE,
  ...
  plotgrid.args = list(nrow = 1),
  annotation.args = list(title = "Elevation distribution by lake connectivity class",
                         subtitle="Lakes with TN data")
)



grouped_gghistostats(
  data = dt1_western_summary %>% drop_na(tn_ugl_median) ,
  x = lake_elevation_m,
  # test.value = 50,
  # type = "nonparametric",
  results.subtitle=FALSE,
...
I was curious whether or not we see any consistent patterns of TP or TN with elevation, and whether those patterns vary depending on the hydrologic connectivity of the waterbody. 
...
dt1_western_summary %>%
  drop_na(tn_ugl_median)%>%
      filter(lake_connectivity_class %in% c("Drainage","DrainageLk",
                                                   "Isolated")) %>%
  ggplot(aes(x=lake_elevation_m, y=log10(tn_ugl_median), color=lake_connectivity_class,
               fill=lake_connectivity_class))+
  geom_point(size=2.5, shape=21, alpha=0.5, color="black")+
 # facet_grid(lake_rsvr_class~lake_connectivity_class)+
  geom_smooth(method="lm",se=F,size=0.5)+
  ...
  scale_fill_discrete_diverging()+
  scale_color_discrete_diverging()+
  ylab("Median TN concentrations (log10 ug/L)")+
  xlab("Elevation (m)")+
  theme(legend.position="right")
```

TN concentrations, on the other hand, show contrary elevation pattern in comparison with TP. TN concentrations decrease as you move up in elevation in the two most abundant connectivity classes and the isolated lakes maintain concentrations trough the elevation gradient. 

## Upstream waterbodies

Another feature of LAGOS both a count and total area of upstream waterbodies. Do upstream waterbodies impact TP and TN concentrations?

```{r echo=FALSE, message=FALSE, warning=FALSE, out.height='50%', out.width='100%'}
dt1_western_summary %>% 
  filter(lake_connectivity_class %in% c("Drainage","DrainageLk",
                                                   "Headwater")) %>%
  dplyr::select(tp_ugl_median,lake_connectivity_class, lake_lakes1ha_upstream_ha,
  ...
  filter(value>0)%>%
  ggplot(aes(x=log10(value), y=log10(tp_ugl_median), fill=lake_connectivity_class, color=lake_connectivity_class))+
...
dt1_western_summary %>% 
  filter(lake_connectivity_class %in% c("Drainage","DrainageLk",
                                                   "Isolated")) %>%
  dplyr::select(tn_ugl_median,lake_connectivity_class, lake_lakes1ha_upstream_ha,
                lake_lakes4ha_upstream_ha, lake_lakes10ha_upstream_ha) %>%
  pivot_longer(-c(1:3))%>%
  mutate(name=factor(name,
                     levels=c("lake_lakes1ha_upstream_ha","lake_lakes4ha_upstream_ha","lake_lakes10ha_upstream_ha"),
                     labels=c("Area >1 ha lakes","Area >4 ha lakes","Area >10 ha lakes"))) %>%
  filter(value>0)%>%
  ggplot(aes(x=log10(value), y=log10(tn_ugl_median), fill=lake_connectivity_class, color=lake_connectivity_class))+
  geom_point(size=2.5, color="black",  alpha=0.5)+
  facet_grid(lake_connectivity_class~name, scales="free_x")+
  # geom_smooth(method="lm",se=F)+
  scale_fill_discrete_diverging()+
...
In DrainageLks, TN concentrations appear to actually increase with greater upstream lake area. 
...
- TP increases and TN decreases with increasing lake elevation
- In DrainageLks you see decreasing upstream lake area with increasing elevation. 
...
dt1_western_summary %>%
  drop_na(tp_ugl_median)%>%
          filter(lake_connectivity_class %in% c("Drainage","DrainageLk",
                                                   "Headwater")) %>%
  ggplot(aes(x=lake_connectivity_fluctuates, y=log10(tn_ugl_median), fill=lake_connectivity_class))+
  geom_boxplot(outlier.shape=NA)+
  #facet_grid(lake_rsvr_class~lake_connectivity_class)+
  scale_fill_discrete_diverging()+
  scale_color_discrete_diverging()+
...
Is there a correlation between TP or TN and WSA:LA? 

```{r, echo=FALSE, message=FALSE, warning=FALSE,out.width='50%', out.height='70%'}
dt1_western_summary %>%
            filter(lake_connectivity_class %in% c("Drainage","DrainageLk",
                                                   "Headwater")) %>%
  drop_na(tn_ugl_median,tp_ugl_median)%>%
    mutate(WALA=(ws_area_ha-lake_totalarea_ha)/lake_totalarea_ha)%>%
...
dt1_western_summary %>%
  ...
  drop_na(tn_ugl_median,tp_ugl_median)%>%
    mutate(WALA=(ws_area_ha-lake_totalarea_ha)/lake_totalarea_ha)%>%
  ggplot(aes(x=log10(WALA), y=log10(tn_ugl_median), color=lake_connectivity_class,fill=lake_connectivity_class))+
  geom_point(shape=21, color="black")+
  scale_fill_viridis_d()+
  scale_color_viridis_d()+
  geom_smooth(method="lm",se=F)+
...
###Keep working from here - Carol note!!
...
dt1_western_summary %>% 
   filter(lake_connectivity_class %in% c("Drainage")) %>%
  wilcox_test(tn_ugl_median ~ lake_rsvr_class) %>%
 # add_significance() %>%
  dplyr::select(.y., p, p.signif) %>%
  mutate(group="Drainage"),
